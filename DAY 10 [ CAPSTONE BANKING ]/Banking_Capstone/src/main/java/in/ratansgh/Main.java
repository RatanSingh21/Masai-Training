...
//    private static final DateTimeFormatter datetimeformatter = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:MM:SS");
    ...
    // Thread pool for multithreaded operations -- 5 threads
    ...
    public static void showmainMenu(){
        ...
        System.out.println("6. Perform Multithreaded Transactions...");
        ...
        switch (choice){
            ...
            case 5:
                ...
                break;
            case 6:
                performMultithreadedTransactions();
                break;
            case 7:
    ...
    private static void createAccount(){
        ...
        if (!RegexValidations.isValidNumeric(balance)){
    ...
    private static void performDeposit() {
        ...
        if (!RegexValidations.isValidNumeric(amountstr)){
...
//            return;
    ...
    private static void viewTransactionHistory(){
        ...
        List<Transaction> accountTransaction = transactions.stream()
                .filter(t -> t.getAccountNo().equals(accountNo)
                        || (t.getToAccountNo() != null && t.getToAccountNo().equals(accountNo)))
    ...
    public static void performMultithreadedTransactions(){
        ...
        int multiThreadChoice = getInput();


        switch (multiThreadChoice){
            case 1:
                multithreadedDeposit();
                break;
            case 2:
                multithreadedWithdraw();
                break;
            case 3:
                multithreadedTransfer();
                break;
            default:
                System.out.println("Invalid Transaction type!! ");
        }
    ...
    private static void multithreadedDeposit() {
        ...
        System.out.print("Enter deposit amount (each thread): ");
        ...
        for (int i = 0; i < threads; i++) {
            tasks.add(() -> {
                ...
                Transaction txn = new Transaction(txnId, amount, accountNo, LocalDateTime.now(), TransactionType.DEPOSIT);
        ...
        try {
            ...
            for (Future<Transaction> f : results) {
                System.out.println("Deposit successful: " + f.get());
            }
        } catch (Exception e) {
            System.out.println("Error during deposits: " + e.getMessage());
        }
    }

    private static void multithreadedWithdraw() {
        System.out.print("Enter account number: ");
        String accountNo = sc.nextLine().trim();
        Account account = accounts.get(accountNo);
        ...
        System.out.print("Enter withdraw amount (each thread): ");
        BigDecimal amount = new BigDecimal(sc.nextLine().trim());
        ...
        for (int i = 0; i < threads; i++) {
            tasks.add(() -> {
                try {
                    account.withdraw(amount);
                    String txnId = generateTransactionID();
                    Transaction txn = new Transaction(txnId, amount, accountNo, LocalDateTime.now(), TransactionType.WITHDRAW);
                    synchronized (transactions) {
                        transactions.add(txn);
                    }
                    return txn;
        ...
        try {
            ...
            for (Future<Transaction> f : results) {
                Transaction t = f.get();
                if (t != null) {
                    System.out.println("Withdraw successful: " + t);
                }
            }
        } catch (Exception e) {
            System.out.println("Error during withdrawals: " + e.getMessage());
        }
    }

    private static void multithreadedTransfer() {
        ...
        System.out.print("Enter transfer amount (each thread): ");
        BigDecimal amount = new BigDecimal(sc.nextLine().trim());
        ...
        for (int i = 0; i < threads; i++) {
            tasks.add(() -> {
                ...
                String txnId = generateTransactionID();
                Transaction txn = new Transaction(txnId, amount, fromAccountNo, LocalDateTime.now(),
                        TransactionType.TRANSFER, toAccountNo);
                synchronized (transactions) {
                    transactions.add(txn);
                }
                return txn;
        ...
        try {
            List<Future<Transaction>> results = executor.invokeAll(tasks);
            for (Future<Transaction> f : results) {
                System.out.println("Transfer successful: " + f.get());
            }
        } catch (Exception e) {
